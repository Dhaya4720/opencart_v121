...
public class TC006_EndToEndTest extends BaseClass {
	...
	hp.enterProductName(pro.getProperty("searchProductName"));
	hp.clickSearch();
	
	SearchPage sp=new SearchPage(driver);
	
	if(sp.isProductExist(pro.getProperty("searchProductName")))
	{
		sp.selectProduct(pro.getProperty("searchProductName"));
		sp.setQuantity("2");
		sp.addToCart();
...
//	myassert.assertEquals(sp.checkConfMsg(),true);
	//Assert.assertFalse(sp.checkConfMsg());
